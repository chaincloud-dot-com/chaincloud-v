...
public class Sha256Hash implements Serializable, Comparable {
    ...
    public static Sha256Hash createDouble(byte[] contents) {
        return new Sha256Hash(BitcoinUtils.doubleDigest(contents));
    }
    ...
    /**
     * Hash code of the byte array as calculated by {@link Arrays#hashCode()}. Note the
     * difference between a SHA256
     * secure bytes and the type of quick/dirty bytes used by the Java hashCode method which is
     * designed for use in
     * bytes tables.
     */
    @Override
    public int hashCode() {
        ...
        return (bytes[31] & 0xFF) | ((bytes[30] & 0xFF) << 8) | ((bytes[29] & 0xFF) << 16) | (
                (bytes[28] & 0xFF) << 24);
    }

    @Override
    public String toString() {
        return BitcoinUtils.bytesToHexString(bytes);
    }
