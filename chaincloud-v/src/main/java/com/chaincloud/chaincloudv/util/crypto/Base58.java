...
public class Base58 {
    ...
    public static CharSequence encodeChecked(byte[] input) {
        ...
        byte[] check = BitcoinUtils.doubleDigest(result, 0, input.length);
...
//    }
    ...
    public static String bas58ToHexWithAddress(String address) throws AddressFormatException {
        ...
        String hex = BitcoinUtils.bytesToHexString(bytes);
        return hex.toUpperCase(Locale.US);
    }

    //added by jjz (bither)
    public static String hexToBase58WithAddress(String hex) {
        byte[] bytes = BitcoinUtils.hexStringToByteArray(hex);
        byte[] checksum = copyOfRange(BitcoinUtils.doubleDigest(bytes), 0, 4);
