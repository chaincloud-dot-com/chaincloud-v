...
public class Bech32 {
    ...
    public static Bech32Data decode(final String str) throws AddressFormatException {
        ...
        for (int i = 0; i < str.length(); ++i) {
            char c = str.charAt(i);
            if (c < 33 || c > 126) throw new AddressFormatException("c, i");
            if (c >= 'a' && c <= 'z') {
                if (upper)
                    throw new AddressFormatException("c, i");
                lower = true;
            }
            if (c >= 'A' && c <= 'Z') {
                if (lower)
        ...
        for (int i = 0; i < dataPartLength; ++i) {
            ...
            if (CHARSET_REV[c] == -1) throw new AddressFormatException("c, i + pos + 1");
            values[i] = CHARSET_REV[c];
        }
        String hrp = str.substring(0, pos).toLowerCase(Locale.ROOT);
        if (!verifyChecksum(hrp, values)) throw new AddressFormatException();
