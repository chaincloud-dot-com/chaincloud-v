...
import static com.chaincloud.chaincloudv.util.crypto.BitcoinUtils.isLessThanUnsigned;
...
public class VarInt {
    ...
    public VarInt(byte[] buf, int offset) {
        ...
        if (first < 253) {
            ...
            // 64 bits.
            val = BitcoinUtils.readUint32(buf, offset + 1) | (BitcoinUtils.readUint32(buf, offset
                    + 5) << 32);
    ...
    public static int sizeOf(long value) {
        if (isLessThanUnsigned(value, 253)) {
            ...
            return 3;  // 1 marker + 2 data bytes
        } else if (isLessThanUnsigned(value, -1L & 0xffffffff & 0xffffffffL)) {
            return 5;  // 1 marker + 4 data bytes
        } else {
